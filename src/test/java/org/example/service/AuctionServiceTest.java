...
public class AuctionServiceTest {
    ...
    private static class MockAuctionRepository implements AuctionRepository {
        ...
        @Override
        public void save(Auction a, String status) {
            store.put(a.getId(), a);
            statuses.put(a.getId(), status);
        }

        @Override
        public void update(Auction a, String status) {
            statuses.put(a.getId(), status);
        }

        @Override
        public void updateStatus(Auction a, String status) {
            statuses.put(a.getId(), status);
        }

        @Override
        public void updateEndTime(Auction a) {
        }

        @Override
        public void updateScheduleAndPrice(Auction a) {
        }
        ...
        @Override
        public List<Auction> findByName(String kw) {
            List<Auction> r = new ArrayList<>();
            store.values().forEach(a -> {
                if (a.getItem().getName().contains(kw)) r.add(a);
            });
            return r;
        }
        ...
        @Override
        public List<Auction> findBySellerId(String id) {
            return new ArrayList<>();
        }

        @Override
        public List<Auction> findWonByBidderId(String id) {
            return new ArrayList<>();
        }
    }
    ...
    private static class MockBidRepository implements BidRepository {
        ...
        @Override
        public void save(BidTransaction bid, int auctionId) {
            bids.add(bid);
        }
    ...
    private static class MockItemRepository implements ItemRepository {
        ...
        @Override
        public void save(Item item, String sellerId) {
            store.put(item.getId(), item);
            sellerMap.put(item.getId(), sellerId);
        }
    ...
    private static class MockAutoBidRepository implements AutoBidRepository {
        @Override
        public void save(AutoBid ab, int auctionId) {
        }
        ...
        @Override
        public PriorityQueue<AutoBid> findActiveByAuction(int id) {
    ...
    private static class MockUserRepository implements UserRepository {
        ...
        @Override
        public void updateBalance(String id, double bal) {
            User u = byId.get(id);
            if (u != null) u.setBalance(bal);
        }

        @Override
        public void updateStatus(String userId, String status) {
            byId.get(userId).setStatus(status);
        }
